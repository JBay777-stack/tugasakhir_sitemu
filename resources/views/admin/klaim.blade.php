    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 animate-slide-up">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">
                Verifikasi bukti kepemilikan dan dokumentasi pengembalian barang.
            </p>
        </div>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase">Bukti</th>
                ...
                @forelse ($claims as $claim)
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <img src="{{ asset('storage/'.$claim->foto_bukti) }}"
                            ...
                            @if($claim->status === 'pending')
                                <div class="flex justify-center gap-2">
                                    <form method="POST" action="/admin/klaim/{{ $claim->id }}/terima">
                                        @csrf
                                        <button class="w-9 h-9 rounded-xl bg-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white">
                                            ✔
                                        </button>
                                    </form>
                                    ...
                                    <form method="POST" action="/admin/klaim/{{ $claim->id }}/tolak">
                                        @csrf
                                        <button class="w-9 h-9 rounded-xl bg-red-100 text-red-600 hover:bg-red-600 hover:text-white">
                                            ✖
                                        </button>
                                    </form>
                                </div>
                            @else
                                <a href="{{ route('admin.klaim.export-pdf', $claim->id) }}"
                                   target="_blank"
